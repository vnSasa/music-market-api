...
func (r *LibraryDB) GetUserToplist(id int) ([]model.SongList, error) {
	var songs []model.SongList
	query := fmt.Sprintf("SELECT s.id, s.artist_id, s.name_song, s.genre, s.second_genre, s.year_of_release, s.rating "+
		"FROM %s s JOIN %s ul ON s.id = ul.song_id WHERE ul.user_id = ?", songTable, topTable)
...
func (r *LibraryDB) AddToToplist(userID, songID int) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	var userIDs []int
	rows, err := r.db.Query("SELECT user_id FROM "+topTable+" WHERE song_id = ?", songID)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	query := fmt.Sprintf("INSERT INTO %s (user_id, song_id) VALUES (?, ?)", topTable)
...
func (r *LibraryDB) DeleteSongFromPlaylist(userID, songID int) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	query := fmt.Sprintf("DELETE FROM %s WHERE user_id=? AND song_id=?", libraryTable)
	_, err = r.db.Exec(query, userID, songID)
	if err != nil {
		return errors.New(err.Error())
	}

	queryTop := fmt.Sprintf("DELETE FROM %s WHERE user_id=? AND song_id=?", topTable)
	_, err = r.db.Exec(queryTop, userID, songID)
	if err != nil {
		return errors.New(err.Error())
	}
...
func (r *LibraryDB) DeleteSongFromToplist(userID, songID int) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	query := fmt.Sprintf("DELETE FROM %s WHERE user_id=? AND song_id=?", topTable)
	_, err = r.db.Exec(query, userID, songID)
	if err != nil {
		return errors.New(err.Error())
	}
